...
RSpec.describe 'Categories New Page', type: :feature do
  ...
  it 'displays the back button' do
    expect(page).to have_link('', href: categories_path, class: 'back') do
      have_selector('i.fas.fa-arrow-left')
    end
  end
  ...
  it 'displays the logout button' do
    expect(page).to have_selector('button[type="submit"].btn-logout') do
      have_selector('i.fa-solid.fa-right-from-bracket')
    end
  end
